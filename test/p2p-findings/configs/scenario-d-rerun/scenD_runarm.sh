...
for run in 1 2; do
  ...
  kubectl exec -n $NS scenc-loadgen -- sh -c \
    "nohup python3 -u /driver/scenD_docqa.py $EP ${ARM}-run${run} 192 128 > /tmp/${ARM}-run${run}.log 2>&1 & echo started"
  last=0
  while true; do
    body=$(kubectl exec -n $NS scenc-loadgen -- cat /tmp/${ARM}-run${run}.log 2>/dev/null)
    n=$(printf '%s\n' "$body" | wc -l | tr -d ' ')
    if [ "$n" -gt "$last" ]; then printf '%s\n' "$body" | tail -n +$((last+1)); last=$n; fi
    printf '%s' "$body" | grep -q "^# done" && break
    alive=$(kubectl exec -n $NS scenc-loadgen -- sh -c 'ls /proc | grep -E "^[0-9]+$" | while read p; do tr "\0" " " < /proc/$p/cmdline 2>/dev/null | grep -q "scenD_docq[a].py" && echo x; done; true' 2>/dev/null | wc -l | tr -d ' ')
    [ "${alive:-0}" -gt 0 ] || { echo "RUN ENDED without done marker"; break; }
    sleep 30
  done
  kubectl exec -n $NS scenc-loadgen -- cat /tmp/${ARM}-run${run}.log > "scenD_${ARM}-run${run}.log" 2>/dev/null
done
