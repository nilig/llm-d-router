#!/bin/bash
# 4-arm GLM A/B on the fixed stack (EPP kv-source-endpoint-92e5de82 +
# sidecar, compensated port bases). Arms are the prior campaign's configs
# minus custom-image-only fields (CM p2p-pd-epp-plugins-v4).
# Per arm: engine-counter snapshot -> glm_arm.sh -> snapshot -> deltas.
# Scales the fleet to 0 at the end regardless of arm outcomes.
set -u
NS=nilig-p2p
...
sumdelta() { # $1 before, $2 after, $3 metric-regex -> prints summed delta
  python3 - "$1" "$2" "$3" <<'PY'
import sys, re
def total(path, rx):
    t = 0.0
...
FAILED=""
run_arm armB          fix-precise      || FAILED="$FAILED fix-precise"
