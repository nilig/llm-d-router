...
for run in 1; do
  ...
  kubectl exec -n $NS scenc-loadgen -- sh -c \
    "nohup python3 -u /driver/scenB64_hotset.py $EP ${ARM} 12,24,36,48 > /tmp/${ARM}.log 2>&1 & echo started"
  last=0
  while true; do
    ...
    alive=$(kubectl exec -n $NS scenc-loadgen -- sh -c 'ls /proc | grep -E "^[0-9]+$" | while read p; do tr "\0" " " < /proc/$p/cmdline 2>/dev/null | grep -q "scenB64_hotse[t].py" && echo x; done; true' 2>/dev/null | wc -l | tr -d ' ')
    [ "${alive:-0}" -gt 0 ] || { echo "RUN ENDED without done marker"; break; }
    sleep 30
  done
  kubectl exec -n $NS scenc-loadgen -- cat /tmp/${ARM}.log > "scenB64_${ARM}.log" 2>/dev/null
done
