...
log "STAGGERED A/B: 5 real staggered forks overlapped, fixed EPP, 3P+1D"
wait_fleet_mf(){
  for i in $(seq 1 240); do
...
log "fleet Ready"
./keep_warm.sh >/tmp/stag-keepwarm.log 2>&1 & KW_PID=$!
for pod in glm-5-2-prefill-0 glm-5-2-prefill-1 glm-5-2-prefill-2 glm-5-2-decode-0; do
  nohup bash -c "while true; do kubectl -n $NS logs $pod -c vllm -f --tail=5 >> /tmp/stag-$pod.log 2>/dev/null; echo \"--- stream ended \$(date -u +%H:%M:%S) ---\" >> /tmp/stag-$pod.log; sleep 5; done" >/dev/null 2>&1 &
done

run stag-base /config/spill-off.yaml staggered-v1

log "full engine restart before p2p arm"
kubectl -n $NS delete pod glm-5-2-prefill-0 glm-5-2-prefill-1 glm-5-2-prefill-2 glm-5-2-decode-0 --wait=false >/dev/null 2>&1
sleep 30; wait_fleet_mf || { log "FAIL: fleet incomplete"; exit 1; }
run stag-p2p /config/spill-on-8650.yaml staggered-v1

log "STAGGERED RESULTS"
for t in stag-base stag-p2p; do
...
[ -f windows/staggered-v1/epp-stag-p2p.jsonl ] && ./.venv/bin/python delta_stats.py windows/staggered-v1/epp-stag-p2p.jsonl --label stag-p2p
pkill -f "stag-glm" 2>/dev/null
log "STAGGERED AB complete"
