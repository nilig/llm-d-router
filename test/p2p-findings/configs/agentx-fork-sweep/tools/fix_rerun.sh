...
log "FIXED-EPP RERUN: mf-p2p-fix (precise) + mfapprox-on-fix (approx) on $NEWIMG"
wait_fleet_mf(){
  for i in $(seq 1 240); do
...
ci=$(kubectl -n $NS get deploy $EPP -o json | python3 -c "
import json,sys
d=json.load(sys.stdin)
print([i for i,c in enumerate(d['spec']['template']['spec']['containers']) if c['name']=='epp'][0])")
kubectl -n $NS patch deploy $EPP --type=json -p "[{\"op\":\"replace\",\"path\":\"/spec/template/spec/containers/$ci/image\",\"value\":\"$NEWIMG\"}]" >/dev/null
kubectl -n $NS rollout status deploy/$EPP --timeout=5m >/dev/null || { log "FAIL: fixed EPP rollout"; exit 1; }
log "fixed EPP live"
...
log "fleet Ready"
./keep_warm.sh >/tmp/fix-keepwarm.log 2>&1 & KW_PID=$!
for pod in glm-5-2-prefill-0 glm-5-2-prefill-1 glm-5-2-prefill-2 glm-5-2-decode-0; do
  nohup bash -c "while true; do kubectl -n $NS logs $pod -c vllm -f --tail=5 >> /tmp/fix-$pod.log 2>/dev/null; echo \"--- stream ended \$(date -u +%H:%M:%S) ---\" >> /tmp/fix-$pod.log; sleep 5; done" >/dev/null 2>&1 &
done

run mf-p2p-fix /config/spill-on-8650.yaml multifork-v1

log "full engine restart before approx arm"
kubectl -n $NS delete pod glm-5-2-prefill-0 glm-5-2-prefill-1 glm-5-2-prefill-2 glm-5-2-decode-0 --wait=false >/dev/null 2>&1
sleep 30; wait_fleet_mf || { log "FAIL: fleet incomplete"; exit 1; }
run mfapprox-on-fix /config/mfapprox-on.yaml multifork-v1

log "summaries vs unfixed baselines"
for t in mf-base mf-p2p-fix mfapprox-off mfapprox-on-fix; do
  ./.venv/bin/python - "$t" <<'PYEOF'
import json, statistics, sys
t=sys.argv[1]
try: recs=[json.loads(l) for l in open(f"windows/multifork-v1/artifacts/{t}/profile_export.jsonl") if l.strip()]
...
vals=sorted(t2 for cid,r in firsts.items()
            if ((r["metrics"].get("input_sequence_length") or {}).get("value") or 0)>=20000
...
for t in mf-p2p-fix mfapprox-on-fix; do
  [ -f windows/multifork-v1/epp-$t.jsonl ] && ./.venv/bin/python delta_stats.py windows/multifork-v1/epp-$t.jsonl --label $t
done
pkill -f "fix-glm" 2>/dev/null
log "FIXED-EPP RERUN complete"
