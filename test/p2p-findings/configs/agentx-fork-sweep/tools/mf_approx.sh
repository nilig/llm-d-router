...
log "APPROX A/B: fitted approximate router +- p2p (gate 8650), multifork-v1, 3P+1D"
wait_fleet_mf(){
  for i in $(seq 1 240); do
...
log "full engine restart for cold state"
...
log "fleet Ready"
./keep_warm.sh >/tmp/mfapprox-keepwarm.log 2>&1 & KW_PID=$!
for pod in glm-5-2-prefill-0 glm-5-2-prefill-1 glm-5-2-prefill-2 glm-5-2-decode-0; do
  nohup bash -c "while true; do kubectl -n $NS logs $pod -c vllm -f --tail=5 >> /tmp/mfapprox-$pod.log 2>/dev/null; echo \"--- stream ended \$(date -u +%H:%M:%S) ---\" >> /tmp/mfapprox-$pod.log; sleep 5; done" >/dev/null 2>&1 &
done

run mfapprox-off /config/mfapprox-off.yaml multifork-v1

log "full engine restart before p2p arm"
kubectl -n $NS delete pod glm-5-2-prefill-0 glm-5-2-prefill-1 glm-5-2-prefill-2 glm-5-2-decode-0 --wait=false >/dev/null 2>&1
sleep 30; wait_fleet_mf || { log "FAIL: fleet incomplete"; exit 1; }
run mfapprox-on /config/mfapprox-on.yaml multifork-v1

log "summaries"
for t in mfapprox-off mfapprox-on; do
...
[ -f windows/multifork-v1/epp-mfapprox-on.jsonl ] && ./.venv/bin/python delta_stats.py windows/multifork-v1/epp-mfapprox-on.jsonl --label mfapprox-on
pkill -f "mfapprox-glm" 2>/dev/null
log "APPROX AB complete"
