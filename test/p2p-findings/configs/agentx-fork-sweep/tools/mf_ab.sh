...
log "MULTIFORK A/B: 6 forks (W=30/15/10/6/5/4) sequential on 3P+1D, per-cold-request attribution"
log "phase 1: fleet up (3 prefill + 1 decode)"
...
wait_fleet_mf(){
  for i in $(seq 1 240); do
    p=$(timeout 30 kubectl -n $NS get lws glm-5-2-prefill -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
    d=$(timeout 30 kubectl -n $NS get lws glm-5-2-decode -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
    [ "${p:-0}" -ge 3 ] && [ "${d:-0}" -ge 1 ] && return 0
    sleep 30
  done
  return 1
}
...
./keep_warm.sh >/tmp/mf-keepwarm.log 2>&1 & KW_PID=$!

run mf-base /config/spill-off.yaml multifork-v1

log "full engine restart before B (decode freshness lesson)"
...
run mf-p2p /config/spill-on-8650.yaml multifork-v1

log "summaries (per-cold-request attribution)"
for t in mf-base mf-p2p; do
...
if vals:
    cold=[v for v in vals if v>=3000]
    warm=[v for v in vals if v<3000]
    b=collections.Counter("<2s" if v<2000 else "2-6s" if v<6000 else "6-12s" if v<12000 else ">=12s" for v in vals)
    print(f"{t}: n={len(vals)} p50={statistics.median(vals):,.0f} p90={vals[int(.9*(len(vals)-1))]:,.0f} max={max(vals):,.0f} buckets={dict(sorted(b.items()))}")
    if cold: print(f"{t}: COLD n={len(cold)} mean={statistics.mean(cold):,.0f} p50={statistics.median(cold):,.0f} | warm n={len(warm)}")
PYEOF
done
[ -f windows/multifork-v1/epp-mf-p2p.jsonl ] && ./.venv/bin/python delta_stats.py windows/multifork-v1/epp-mf-p2p.jsonl --label mf-p2p
log "MULTIFORK campaign complete"
