...
log "FIXED-EPP BASELINES: mf-base-fix (precise off) + mfapprox-off-fix (approx off)"
wait_fleet_mf(){
  for i in $(seq 1 240); do
...
log "fleet Ready"
./keep_warm.sh >/tmp/bfix-keepwarm.log 2>&1 & KW_PID=$!

run mf-base-fix /config/spill-off.yaml multifork-v1

log "full engine restart before approx baseline"
...
run mfapprox-off-fix /config/mfapprox-off.yaml multifork-v1

log "FULL 2x2 TABLE (fixed EPP)"
for t in mf-base-fix mf-p2p-fix mfapprox-off-fix mfapprox-on-fix; do
...
pkill -f "bfix-glm" 2>/dev/null
log "BASELINES-FIX complete"
